# state-machine-multi-market — Multi-Market State Machine Engine

> A production-ready, stateless, table-driven state machine framework with multi-market support.
> Multi-module Maven project: `statemachine-core` (shared engine), `chat-engine` (Conversation SM), `agent-connector` (Interaction SM).
> Features: YAML config with three-layer inheritance, market routing & isolation, config validation, market-specific action strategies.
...
### Multi-Market Support (chat-engine)

This project includes comprehensive multi-market support for deploying the state machine across multiple regions (HK, UK, SG, etc.) with shared main flow but market-specific configurations.

| Feature | Package | Key Class | Description |
|---------|---------|-----------|-------------|
| **YAML Config Loading** | `config.loader` | `YamlMarketConfigLoader` | Load market config from YAML files (classpath or filesystem) |
| **Three-Layer Inheritance** | `config.loader` | `ThreeLayerConfigResolver` | base → regional → market config inheritance with merge |
| **Config Repository** | `config` | `MarketConfigRepository` | Cached config per market with hot-reload support |
| **Market Router** | `routing` | `MarketRouter` | Route requests to appropriate market config with default fallback |
| **Circuit Breaker** | `routing` | `MarketCircuitBreaker` | Per-market circuit breaker to isolate failures (CLOSED/OPEN/HALF_OPEN) |
| **Config Validation** | `config.validation` | `ConfigValidator` | Validate config for correctness with ERROR/WARNING/INFO levels |
| **Transfer Strategies** | `action.strategy` | `TransferStrategyRegistry` | Market-specific transfer logic (Genesys/Internal/AIBot) via strategy pattern |

#### Config Directory Structure

```
src/main/resources/config/
├── base-profile.yaml              # Global defaults (all markets inherit)
├── regional-profiles/
│   ├── apac.yaml                  # APAC regional overrides (HK, SG, JP, AU)
│   ├── emea.yaml                  # EMEA regional overrides (UK, DE, FR)
│   └── amer.yaml                  # AMER regional overrides (US, CA, BR)
└── market-profiles/
    ├── hk.yaml                    # HK-specific overrides
    ├── sg.yaml                    # SG-specific overrides
    └── uk.yaml                    # UK-specific overrides
```
...
#### Extended StateMachineMarketConfig

| Category | Fields |
|----------|--------|
| **Timeouts** | customerIdleSeconds, transferTimeoutSeconds, endingGraceSeconds, surveyTimeoutSeconds |
| **Feature Toggles** | surveyEnabled, transferEnabled, genesysEnabled, aibotEnabled, regulatoryAuditEnabled |
| **Business Rules** | fallbackRoutingStrategy, surveyType, transferTarget |
| **Retry** | maxRetries, retryBaseDelayMs, retryMaxDelayMs |
| **Connectors** | aibotEndpoint, genesysOrgId, websocketEndpoint |
| **Extensions** | enabledExtensions (list of market-specific extension names) |
