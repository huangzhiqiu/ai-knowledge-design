...
/**
 * Aspect for logging agent connector state machine execution.
 * <p>
...
public class StateMachineLoggingAspect {
    ...
    /**
     * Pointcut for all methods in AgentConnectorStateMachineService and its subclasses.
     */
    @Pointcut("execution(* com.selfdevelopment.agentconnector.service.AgentConnectorStateMachineService.*(..))")
    public void stateMachineServiceMethods() {
        // Pointcut definition - no body needed
    }

    /**
     * Pointcut for all methods in SpringAgentConnectorStateMachineService.
     */
    @Pointcut("execution(* com.selfdevelopment.agentconnector.spring.service.SpringAgentConnectorStateMachineService.*(..))")
    ...
    public Object logStateMachineExecution(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Entering agent connector state machine method: {} with {} arguments",
        ...
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Exiting agent connector state machine method: {} returned {} in {}ms",
                        methodName,
            ...
            if (duration > 100) {
                log.warn("Slow agent connector state machine operation: {} took {}ms (threshold: 100ms)",
                        methodName, duration);
            }
            ...
            // Log exception
            log.error("Exception in agent connector state machine method: {} after {}ms: {}",
