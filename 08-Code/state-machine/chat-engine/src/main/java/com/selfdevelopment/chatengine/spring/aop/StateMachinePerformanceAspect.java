...
/**
 * Aspect for monitoring state machine performance metrics.
 * <p>
...
public class StateMachinePerformanceAspect {
     ...
     */
    @Pointcut("execution(* com.selfdevelopment.chatengine.service.ChatEngineStateMachineService.fire(..))")
    ...
    public Object monitorPerformance(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            if (durationMs > VERY_SLOW_OPERATION_THRESHOLD_MS) {
                log.error("VERY SLOW state machine operation: {} took {}ms (threshold: {}ms)",
                        methodName, durationMs, VERY_SLOW_OPERATION_THRESHOLD_MS);
            } else if (durationMs > SLOW_OPERATION_THRESHOLD_MS) {
                log.warn("Slow state machine operation: {} took {}ms (threshold: {}ms)",
            ...
            log.error("State machine operation failed: {} took {}ms, error: {}",
    ...
    private void logPerformanceSummary(String methodName, MethodMetrics metrics) {
        log.info("Performance summary for {}: total={}, success={}, failures={}, " +
