...
> **Survey as in-progress state**: When `surveyEnabled=true`, `closeConversation()` fires `SURVEY_START` to enter `IN_PROGRESS` instead of directly going to `ENDING`. The survey flow is fully controlled by the state machine.
...
|----|--------|-------|--------|-------|--------|---------|---------|
| T01 | INITIATED | CUSTOMER_CONNECT | IN_PROGRESS | - | - | - | Customer establishes connection |
| T02 | IN_PROGRESS | TRANSFER_REQUEST | TRANSFERRED | transferEnabled | - | - | Request human agent |
| T03 | TRANSFERRED | TRANSFER_CONNECTED | IN_PROGRESS | - | - | - | Reserved for future |
| T04 | TRANSFERRED | TRANSFER_FAILED | INITIATED | - | - | - | **v6: no rollback to IN_PROGRESS** |
| T05 | TRANSFERRED | TRANSFER_TIMEOUT | INITIATED | - | - | - | **v6: no rollback to IN_PROGRESS** |
| T06 | IN_PROGRESS | CUSTOMER_CLOSE | ENDING | !surveyEnabled | - | - | Customer closes, no survey |
...
| T12 | IN_PROGRESS | SURVEY_START | IN_PROGRESS | surveyEnabled | Show survey UI | - | **Survey as in-progress state** |
| T13 | TRANSFERRED | SURVEY_START | IN_PROGRESS | surveyEnabled | Show survey UI | - | **Survey as in-progress state** |
| T14 | IN_PROGRESS | SURVEY_COMPLETE | ENDING | - | Save survey results | - | Survey completed normally |
| T15 | IN_PROGRESS | SYS_SURVEY_TIMEOUT | ENDING | - | Save partial results | SurveyTimeoutMonitor | Survey timed out |
| T16 | IN_PROGRESS | SYS_CUSTOMER_IDLE | ENDING | - | - | CustomerIdleMonitor | Customer left during survey |
| T17 | IN_PROGRESS | CUSTOMER_CLOSE | ENDING | - | - | - | Customer explicitly closes survey |
| T18 | INITIATED | SYS_ACTION_FAILED | ERROR | - | Log error, alert | FailoverStateMachine | **Failover: action error** |
| T19 | IN_PROGRESS | SYS_ACTION_FAILED | ERROR | - | Log error, alert | FailoverStateMachine | **Failover: action error** |
| T20 | TRANSFERRED | SYS_ACTION_FAILED | ERROR | - | Log error, alert | FailoverStateMachine | **Failover: action error** |
| T21 | IN_PROGRESS | SYS_ACTION_FAILED | ERROR | - | Log error, alert | FailoverStateMachine | **Failover: action error** |
| T22 | ERROR | SYS_RETRY | IN_PROGRESS | - | Re-initialize resources | - | Manual or system retry |
...
| IN_PROGRESS | Show survey UI, start survey timer | Save survey results, stop timer |
